...
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS):
    ...
    for it, target in enumerate(dataloader_validate):
        ...
        os.makedirs(out_dir + "/validate_first_run", exist_ok=True)
        for k in result_dict.keys():
            np_img = result_dict[k].detach().cpu().numpy()
            util.save_image(
                os.path.join(out_dir, "validate_first_run") +
                "/" +
...
if __name__ == "__main__":
    ...
    FLAGS.dmtet_grid = 256  # Resolution of initial tet grid. We provide 64 and 128 resolution grids. Other resolutions can be generated with https://github.com/crawforddoran/quartet
