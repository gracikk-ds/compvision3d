...
class _MLP(torch.nn.Module):
    def __init__(self, cfg, loss_scale=1.0):
        ...
        net = (
            torch.nn.Linear(cfg["n_input_dims"], cfg["n_neurons"], bias=False),
            torch.nn.ReLU(),
        )
        for i in range(cfg["n_hidden_layers"] - 1):
            net = net + (
                torch.nn.Linear(cfg["n_neurons"], cfg["n_neurons"], bias=False),
                torch.nn.ReLU(),
            )
        net = net + (
            torch.nn.Linear(cfg["n_neurons"], cfg["n_output_dims"], bias=False),
        )
...
class MLPTexture3D(torch.nn.Module):
    def __init__(self, AABB, channels=3, internal_dims=32, hidden=2, min_max=None):
        ...
        self.encoder.register_full_backward_hook(
            lambda module, grad_i, grad_o: (grad_i[0] / gradient_scaling,)
        )
    ...
    def sample(self, texc):
        ...
        out = (
            torch.sigmoid(out) * (self.min_max[1][None, :] - self.min_max[0][None, :])
            + self.min_max[0][None, :]
        )
