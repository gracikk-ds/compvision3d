...
class texture2d_mip(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        gy, gx = torch.meshgrid(
            torch.linspace(
                0.0 + 0.25 / dout.shape[1],
                1.0 - 0.25 / dout.shape[1],
                dout.shape[1] * 2,
                device="cuda",
            ),
            torch.linspace(
                0.0 + 0.25 / dout.shape[2],
                1.0 - 0.25 / dout.shape[2],
                dout.shape[2] * 2,
                device="cuda",
            ),
            indexing="ij",
...
class Texture2D(torch.nn.Module):
    ...
    def __init__(self, init, min_max=None):
        ...
        if isinstance(init, list):
            self.data = list(
                torch.nn.Parameter(mip.clone().detach(), requires_grad=True)
                for mip in init
            )
            ...
            self.data = torch.nn.Parameter(
                init[None, ...].clone().detach(), requires_grad=True
            )
    ...
    def sample(self, texc, texc_deriv, filter_mode="linear-mipmap-linear"):
        if isinstance(self.data, list):
            ...
            if self.data.shape[1] > 1 and self.data.shape[2] > 1:
                ...
                out = dr.texture(
                    mips[0], texc, texc_deriv, mip=mips[1:], filter_mode=filter_mode
                )
            else:
    ...
    with torch.no_grad():
        ...
        if not auto_mipmaps:
            ...
            while mip_chain[-1].shape[1] > 1 or mip_chain[-1].shape[2] > 1:
                new_size = [
                    max(mip_chain[-1].shape[1] // 2, 1),
                    max(mip_chain[-1].shape[2] // 2, 1),
                ]
