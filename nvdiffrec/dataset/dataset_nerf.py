...
def _load_img(path):
    ...
    assert len(files) > 0, "Tried to find image file for: %s, but found 0 files" % (
        path
    )
...
class DatasetNERF(Dataset):
    def __init__(self, cfg_path, FLAGS, examples=None):
        ...
        self.resolution = _load_img(
            os.path.join(self.base_dir, self.cfg["frames"][0]["file_path"])
        ).shape[0:2]
        self.aspect = self.resolution[1] / self.resolution[0]

        if self.FLAGS.local_rank == 0:
            print(
                "DatasetNERF: %d images with shape [%d, %d]"
    ...
    def _parse_frame(self, cfg, idx):
        ...
        proj = util.perspective(
            fovy, self.aspect, self.FLAGS.cam_near_far[0], self.FLAGS.cam_near_far[1]
        )

        # Load image data and modelview matrix
        img = _load_img(os.path.join(self.base_dir, cfg["frames"][idx]["file_path"]))
        mv = torch.linalg.inv(
            torch.tensor(cfg["frames"][idx]["transform_matrix"], dtype=torch.float32)
        )
